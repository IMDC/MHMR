...
export const getSentimentFromChatGPT = async (transcript) => {
  const inputText = `Analyze the sentiment of this video transcript and return one of the following labels: Very Negative, Negative, Neutral, Positive, or Very Positive. Transcript: "${transcript}"`;
  const data = await connectToChatGPT(inputText);
  if (data.choices && data.choices.length > 0) {
